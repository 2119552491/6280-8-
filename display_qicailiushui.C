...
//		if ( time_led_speed_ms_tick >=  6 ) // 255 led_speed_Max led_level LED_LEVEL_Max RTC_mode_15_count
		...
		//	time_led_speed_ms_tick = 0;
			...
			while( led_num -- ) //  send_data_table_num
			{
				...
				/*
				temp_byte_value=HC_G_LED_Color_LEVEL_TABLE1[Color_mode];
			//	LED_PWM_16_LEVEL_TABLE();
				send_one_byte();//

				temp_byte_value=HC_R_LED_Color_LEVEL_TABLE1[Color_mode];
			//	LED_PWM_16_LEVEL_TABLE();
				send_one_byte();//

				temp_byte_value=HC_B_LED_Color_LEVEL_TABLE1[Color_mode];
			//	LED_PWM_16_LEVEL_TABLE();
				send_one_byte();//
				*/
			}
...
//			RTC_mode_10_step++;
			...
			if( RTC_mode_10_step == 0 || RTC_mode_10_step == 4 )// LED_G_PWM=255;
			{
					LED_R_PWM-=fade_Time_Count;
			}
			else if( RTC_mode_10_step == 1  )	// LED_G_PWM=255;  || RTC_mode_10_step == 4
			{
					LED_R_PWM+=fade_Time_Count;
			}
			else if( RTC_mode_10_step == 2 )	// LED_G_PWM=255;
			{
					LED_G_PWM-=fade_Time_Count;
			}
			else if( RTC_mode_10_step == 3 )	// LED_G_PWM=255;
			{
					LED_B_PWM+=fade_Time_Count;
			}
			else if( RTC_mode_10_step == 6 )	// LED_G_PWM=255;
			{
					LED_B_PWM-=fade_Time_Count;
			}
			else if( RTC_mode_10_step == 5  )	// LED_G_PWM=255;  || RTC_mode_10_step == 4
			{
					LED_R_PWM+=fade_Time_Count;
					LED_G_PWM+=fade_Time_Count;
			}
